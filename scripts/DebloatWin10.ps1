...
Function Start-Debloat {
    ...
    [regex]$WhitelistedApps = 'Microsoft.ScreenSketch|Microsoft.Paint3D|Microsoft.WindowsCalculator|Microsoft.WindowsStore|Microsoft.Windows.Photos|CanonicalGroupLimited.UbuntuonWindows|Microsoft.MicrosoftStickyNotes|Microsoft.MSPaint|Microsoft.WindowsCamera|.NET|Framework|Microsoft.HEIFImageExtension|Microsoft.StorePurchaseApp|Microsoft.VP9VideoExtensions|Microsoft.WebMediaExtensions|Microsoft.WebpImageExtension|Microsoft.DesktopAppInstaller'
...
Function Protect-Privacy {
    ...
    #Creates a PSDrive to be able to access the 'HKCR' tree (already created at file load, so ignore if it exists)
...
Function Stop-EdgePDF {
    ...
    If (!(Get-ItemProperty $NoPDF NoOpenWith -ErrorAction SilentlyContinue)) {
        New-ItemProperty $NoPDF NoOpenWith -ErrorAction SilentlyContinue
    }
    If (!(Get-ItemProperty $NoPDF NoStaticDefaultVerb -ErrorAction SilentlyContinue)) {
        New-ItemProperty $NoPDF NoStaticDefaultVerb -ErrorAction SilentlyContinue
    }
    If (!(Get-ItemProperty $NoProgids NoOpenWith -ErrorAction SilentlyContinue)) {
        New-ItemProperty $NoProgids NoOpenWith -ErrorAction SilentlyContinue
    }
    If (!(Get-ItemProperty $NoProgids NoStaticDefaultVerb -ErrorAction SilentlyContinue)) {
        New-ItemProperty $NoProgids NoStaticDefaultVerb -ErrorAction SilentlyContinue
    }
    If (!(Get-ItemProperty $NoWithList NoOpenWith -ErrorAction SilentlyContinue)) {
        New-ItemProperty $NoWithList NoOpenWith -ErrorAction SilentlyContinue
    }
    If (!(Get-ItemProperty $NoWithList NoStaticDefaultVerb -ErrorAction SilentlyContinue)) {
        New-ItemProperty $NoWithList NoStaticDefaultVerb -ErrorAction SilentlyContinue
    }
}
